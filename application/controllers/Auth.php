...
class Auth extends CI_Controller
{
	...
	public function index()
	{
		...
		else
		{
			...
			$this->_render_page('santri' . DIRECTORY_SEPARATOR . 'data_santri', $this->data);
		}
	}
	...
	public function create_user()
	{
			...
			if ($_FILES['photo']['error'] <> 4)
			{
				...
				else
				{
					...
					$additional_data = [
						...
						'kelas_madin_id' => $this->input->post('kelas_madin'),
		...
		if ($this->form_validation->run() === TRUE && $this->ion_auth->register($identity, $password, $email, $additional_data))
		{
			...
			<i class="icon fas fa-check"></i> Data berhasil ditambahkan</div>');
			redirect("auth", 'refresh');
		}
		else
		{
			...
			$this->data['kelas'] = $this->Kelas_model->get_kelas();
			$this->data['g_santri'] = array('-- Pilih Putra/Putri --','Putra', 'Putri');
			$this->data['provinsi'] = $this->Wilayah_model->get_provinsi();

			$this->_render_page('santri' . DIRECTORY_SEPARATOR . 'add_santri', $this->data);
		}
	}
	...
	public function pilih_kota()
	{
		...
		$this->load->view('santri/kota',$this->data);
	}

	public function pilih_kecamatan()
	{
		$this->data['kecamatan']=$this->Wilayah_model->get_kecamatan($this->uri->segment(3));
		$this->load->view('santri/kecamatan',$this->data);
	}
	/**
	...
	*/
	...
	public function edit_user($id)
	{
		...
		// validate form input
		$this->form_validation->set_rules('name', $this->lang->line('create_user_validation_fname_label'), 'trim|required');
		$this->form_validation->set_rules('username', $this->lang->line('create_user_validation_identity_label'), 'trim|required');
		...
		if (isset($_POST) && !empty($_POST))
		{
			...
			if ($this->form_validation->run() === TRUE)
			{
				if ($_FILES['photo']['error'] <> 4)
				{	
					$delete = $this->Ion_auth_model->del_photo_user($this->input->post('id'));
				...
				if ($this->input->post('kelas_madin'))
				{
					$data['kelas_madin_id'] = $this->input->post('kelas_madin');
				}
				if ($this->input->post('provinsi_id'))
				{
					$data['provinsi'] = $this->input->post('provinsi_id');
				}
				if ($this->input->post('kota_id'))
				{
					$data['kota'] = $this->input->post('kota_id');
				}
				if ($this->input->post('kecamatan_id'))
				{
					$data['kecamatan'] = $this->input->post('kecamatan_id');
				}
				if ($this->input->post('tgl_lahir'))
				{
					$data['lahir'] = $this->input->post('kecamatan_id');
				}
		...
		$this->data['tgl_lahir'] = [
			...
			'data-inputmask-alias' => 'datetime',
			'data-inputmask-inputformat' => "yyyy/mm/dd",
			'value' => $this->form_validation->set_value('tgl_lahir', $user->tgl_lahir),
		];

		$this->data['kelas'] = $this->Kelas_model->get_kelas();
		$this->data['provinsi'] = $this->Wilayah_model->get_provinsi();

		$this->_render_page('santri/edit_santri', $this->data);
	}

	public function edit_user_o($id)
	{
		$this->data['title'] = $this->lang->line('edit_user_heading');
		...
		// validate form input
		$this->form_validation->set_rules('name', $this->lang->line('edit_user_validation_fname_label'), 'trim|required');
		$this->form_validation->set_rules('username', $this->lang->line('edit_user_validation_lname_label'), 'trim|required');
		...
		$this->data['kode_pos'] = [
			...
			'id'    => 'kode_pos',
			'type'  => 'text',
			'value' => $this->form_validation->set_value('company', $user->kode_pos),
		];
		$this->data['phone'] = [
			'name'  => 'phone',
			'id'    => 'phone',
	...
	{
		$delete = $this->Ion_auth_model->del_photo_user($id);
		...
		if ($delete)
		{
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success alert-dismissible">
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
			<i class="icon fas fa-check"></i> Data berhasil dihapus</div>');
		...
		else
		{
			$this->session->set_flashdata('message', '<div class="alert alert-warning alert-dismissible">
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
			<i class="icon fas fa-exclamation-triangle"></i> Data tidak ditemukan</div>');
